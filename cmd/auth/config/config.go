...
type Config struct {
	...
	HTTP struct {
		...
		GRPC struct {
			Host          string        `mapstructure:"host"`
			Port          int           `mapstructure:"port"`
			ServerMinTime time.Duration `mapstructure:"server_min_time"`
			ServerTime    time.Duration `mapstructure:"server_time"`
	...
	Encryption struct {
		EmailPassphrase    string `mapstructure:"email_passphrase"`
		PasswordPassphrase string `mapstructure:"password_passphrase"`
...
func GetConfigJSON(logger *zap.SugaredLogger) *Config {
	...
	pathConfig := fmt.Sprintf("cmd/auth/config/app/%s.json", appEnv)
